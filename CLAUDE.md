...
> 最后更新: 2026-05-12 | 状态: 阶段 A+B+C 完成, 阶段 D 完成 (4/4 场景), 阶段 E 完成
...
| 步骤 | 任务 | 状态 | 备注 |
|------|------|------|------|
| D1 | 测试 cubicle (640 帧) | ✅ 完成 | frame_skip=10 (64帧), 35 mesh_places, 138K mesh pts (旧版) |
| D1b | 测试 cubicle (MALLOC_ARENA_MAX=2) | ✅ 完成 | frame_skip=200 (4帧), 15 mesh_places, 66K mesh pts |
| D2 | 测试 apartment (1,168 帧) | ✅ 完成 | frame_skip=100 (12帧), 69 mesh_places, 33K mesh pts |
| D3 | 测试 office (1,460 帧) | ✅ 完成 | frame_skip=200 (8帧), 305 mesh_places, 93K mesh pts |
| D4 | 测试 building (3,843 帧) | ✅ 完成 | frame_skip=400 (10帧), 598 mesh_places, 253K mesh pts |

### 阶段 E: 评估

| 步骤 | 任务 | 状态 | 备注 |
|------|------|------|------|
| E1 | 编写 IoU 计算脚本 (物体 bbox) | ✅ 完成 | `eval/eval_clio.py` — 房间分配 + 物体邻近度 + 2D IoU |
| E2 | 编写房间分割评估脚本 | ✅ 完成 | 同上脚本，包含 room assignment 和 room centroid coverage |
| E3 | 汇总评估结果 | ✅ 完成 | `eval/summarize_evals.py` — 全场景对比汇总 |
...
| 8 | TSDF 集成极慢（250 线程 futex 争用） | hydra (运行时) | glibc malloc 在 128 核机器上创建过多 arena (>100)，250 线程争用致每帧 >5 分钟 | **运行时必需**: `export MALLOC_ARENA_MAX=2` (见下方运行时环境变量) |
...
运行 `run_clio_pipeline` **必须**设置以下环境变量，否则 TSDF 集成会极慢（每帧 >5 分钟）：

```bash
# 解决 128 核机器上 glibc malloc arena 争用问题
export MALLOC_ARENA_MAX=2

# 动态库路径
export LD_LIBRARY_PATH=/home/DazhiHuang/catkin_ws/devel/lib:/home/DazhiHuang/anaconda3/envs/ros-noetic/lib:$LD_LIBRARY_PATH
```
...
| Scene | Frames (skip) | Places | Edges | Mesh Pts | Mesh Faces |
|-------|--------------|--------|-------|----------|-------------|
| cubicle | 4 (200) | 15 | N/A | 66,551 | 75,460 |
| cubicle | 64 (10) | 35 | 39 | 138,614 | 175,090 |
| apartment | 12 (100) | 69 | 95 | 33,228 | 38,758 |
| office | 8 (200) | 305 | 455 | 93,612 | 104,618 |
| building | 10 (400) | 598 | 847 | 253,054 | 288,206 |

### 房间分配评估

| Scene | GT Rooms | Rooms Covered | Place Assignment | Room Centroid Dist (mean) |
|-------|----------|---------------|------------------|---------------------------|
| apartment | 3 | 2/3 (67%) | 97.1% | 1.34m |
| office | 8 (10 bboxes) | 5/8 (63%) | 54.4% | 1.82m |
| building | 13 (15 bboxes) | 7/13 (54%) | 98.0% | 2.92m |

### 物体邻近度评估

| Scene | GT Objects | <0.5m | <1.0m | <2.0m | Mean Dist |
|-------|-----------|-------|-------|-------|-----------|
| apartment | 29 | 24.1% | 89.7% | 100% | 0.69m |
| office | 33 | 9.1% | 39.4% | 81.8% | 1.50m |
| building | 0 | — | — | — | (无 GT) |

### 已知局限

1. **仅 mesh_places 层有节点**: DSG layers 2 (objects), 3 (places), 4 (rooms), 5 (buildings) 均为空。当前 label space 仅 2 个标签 (unknown + building)，不足以区分物体和房间。需要集成语义推理模块来获取更丰富的标签。
2. **TSDF 集成速度受 MALLOC_ARENA_MAX 影响**: 128 核服务器上 glibc malloc arena 争用导致每帧 ~31 秒。设置 `MALLOC_ARENA_MAX=2` 后修复。
3. **相机内参可能不匹配**: 内参来自 640x360 配置但 image 为 640x480。需要验证并可能调整 cy 参数。
4. **building 任务标注为空**: 25 个任务均无 3D bbox，无法计算物体邻近度。
5. **cubicle 缺少 rooms annotation**: 无法评估房间分配。

## 10. 下一步行动 (下次工作)

### 当前: 完成所有场景构建 + 运行评估 (阶段 D + E)

cubicle 已确认成功（MALLOC_ARENA_MAX=2 修复后）。apartment/office/building 当前在后台运行中。

1. 等待 apartment/office/building 完成 (frame_skip=100/100/300)
2. 运行全场景评估: `bash eval/run_all_evals.sh`
3. 查看汇总: `python3 eval/summarize_evals.py /tmp/clio_output`
4. 如果评估结果理想，任务完成；否则调整配置并重试

### 场景处理命令模板:
```bash
export LD_LIBRARY_PATH=/home/DazhiHuang/catkin_ws/devel/lib:/home/DazhiHuang/anaconda3/envs/ros-noetic/lib
export MALLOC_ARENA_MAX=2
...
### 评估命令:
```bash
python3 eval/eval_clio.py --dsg /tmp/clio_output/<scene>/dsg.json \
    --rooms /data/YueChang/Clio/<scene>/rooms_<scene>.yaml \
    --tasks /data/YueChang/Clio/<scene>/tasks_<scene>.yaml \
    --output /tmp/clio_output/<scene>/eval_results.json
```
